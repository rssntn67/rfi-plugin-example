...
@Component(name = "scairEventIngestor",
        immediate = true)
public class ScairEventIngestor implements EventListener {

    private static final Logger log = LoggerFactory.getLogger(ScairEventIngestor.class);

    private static final String UEI_SCAIR_PREFIX = "uei.opennms.org/traps/SCAIR-MIB-ALARM";
    ...
    public static final List<String> INTERESTING_SCAIR_UEIS = Arrays.asList(
            ...
            UEI_SCAIR_PREFIX + "/scairEventTrapAlarmMinor",
            UEI_SCAIR_PREFIX + "/scairEventTrapAlarmMajor",
            UEI_SCAIR_PREFIX + "/scairEventTrapAlarmCritical",
            UEI_SCAIR_PREFIX + "/scairEventTrapClear",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationIndeterminate",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationInformational",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationWarning",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationMinor",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationMajor",
            UEI_SCAIR_PREFIX + "/scairEventTrapNotificationCritical"
    );
    ...
    public void activate(){
        eventSubscriptionService.addEventListener(this, INTERESTING_SCAIR_UEIS);
        log.info("ScairEventIngestor registered on UEIS: {}", INTERESTING_SCAIR_UEIS);
    }

    @Deactivate
    public void deactivate(){
        try {
            eventSubscriptionService.removeEventListener(this, INTERESTING_SCAIR_UEIS);
            log.info("ScairEventIngestor deregistered");
        } catch (Exception e) {
    ...
    public void onEvent(InMemoryEvent e) {
        ...
        log.info("Arrived new event filtered to ScairEventIngestorComponent: {}", e);
    ...
    protected ImmutableInMemoryEvent translate(InMemoryEvent e) {
        ...
        String nodeLabel = e.getParametersByName(NODE_LABEL_SCAIR_PARAMETER_MATCH).stream()
        ...
        String timeEvent = e.getParametersByName(TIME_SCAIR_PARAMETER).stream()
                .findFirst().map(EventParameter::getValue).orElse(null);
